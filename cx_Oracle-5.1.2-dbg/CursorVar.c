    ...
    udt_Cursor *cursor)                 // cursor created by
{
    ...
    if (!var->cursors)
        return -1;
    PySys_WriteStderr("CursorVar_Initialize conn=%x ae=%d\n", var->connection, var->allocatedElements);
    for (i = 0; i < var->allocatedElements; i++) {
        ...
        PySys_WriteStderr("set position %d in data to %x\n", i, var->data[i]);
    }
