...
#define DRIVER_NAME             "cx_Oracle_dbg-"BUILD_VERSION_STRING
    ...
    PyObject *baseException)            // exception to base exception on
{
    ...
    sprintf(buffer, "cx_Oracle_dbg.%s", name);
...
#else
    module = Py_InitModule("cx_Oracle_dbg", g_ModuleMethods);
#endif
    if (!module)
        return NULL;
...
//-----------------------------------------------------------------------------
#if PY_MAJOR_VERSION >= 3
PyMODINIT_FUNC PyInit_cx_Oracle_dbg(void)
{
    return Module_Initialize();
}
#else
void initcx_Oracle_dbg(void)
{
    Module_Initialize();
}
#endif
