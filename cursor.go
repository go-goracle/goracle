...
type Cursor sturct {
    ...
    arraySize, bindArraySize, fetchArraySize, setInputSizes, outputSize, outputSizeColumn, rowCount,actualRows, rownum,statementType int
...
func (cur *Cursor) allocateHandle *Error {
	cur.isOwned = true
	return ociHandleAlloc(unsafe.Pointer(cur.environment.handle), C.OCI_HTYPE_STATEMENT,
...
func (cur *Cursor) GetBindNames(numElements int) (names []string, err error) {
    ...
    elementSize = (C.sizeof(*C.char) + C.sizeof(C.ub1) + C.sizeof(C.*char)
    	+ C.sizeof(C.ub1) + C.sizeof(C.ub1) + C.sizeof(*C.OCIBind))
    buffer := make([]byte, numElements * elementSize)
...
func (cur *Cursor) PerformDefine() error {
    ...
    if err := CheckStatus(C.OCIAttrGet(unsafe.Pointer(cur.handle), C.OCI_HTYPE_STMT,
    	unsafe.Pointer(&numParams), 0,
            C.OCI_ATTR_PARAM_COUNT, cur.environment.errorHandle); err != nil {
    	return err
    }
